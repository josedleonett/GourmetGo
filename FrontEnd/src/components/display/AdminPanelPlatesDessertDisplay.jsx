...
function AdminPanelPlatesDessertDisplay() {
  ...
  const [newDrinkData, setNewDrinkData] = useState({ name: '', image: '', price: 0, imageFile: null });
  ...
  const handleEdit = (drinkToEdit) => {
    ...
    setNewDrinkData({ name: drinkToEdit.name, image: '', price: drinkToEdit.price, imageFile: null });
  };
  ...
  const handleNewDrinkChange = (event, field) => {
    if (field === 'image') {
      setNewDrinkData({ ...newDrinkData, image: event.target.value, imageFile: event.target.files[0] });
    } else {
  ...
  return (
    <div style={{ height: 400, width: '100%' }}>
      <div style={{ marginBottom: '10px' }}>
        <div>
          <label>Name:</label>
          <input type="text" value={newDrinkData.name} onChange={(e) => handleNewDrinkChange(e, 'name')} />
        </div>
        <div>
          <label>Image:</label>
          <input type="file" onChange={(e) => handleNewDrinkChange(e, 'image')} />
        </div>
        <div>
          <label>Price:</label>
          <input type="number" value={newDrinkData.price} onChange={(e) => handleNewDrinkChange(e, 'price')} />
        </div>
        {editMode ? (
