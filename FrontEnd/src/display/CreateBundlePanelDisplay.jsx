...
import { Button, Checkbox, FormControlLabel, FormGroup, TextField, Paper, Typography, Input, InputLabel } from '@mui/material';
...
const CreateBundlePanelDisplay = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setMainCourses(plateRes.data.filter(item => item.type === 'mainCourse'));
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('http://localhost:8080/v1/bundle/create', formData, {
    ...
    return (
        <StyledPaper>
            <Typography variant="h5" gutterBottom>Create Bundle</Typography>
            <form onSubmit={handleSubmit}>
                <TextField name="name" placeholder="Name" onChange={handleInputChange} fullWidth sx={{ mb: 2 }} />
                <TextField name="description" placeholder="Description" onChange={handleInputChange} fullWidth sx={{ mb: 2 }} />
...
{[
    ...
    <FormGroup sx={{ mb: 2 }} key={group.name}>
        <Typography variant="h6">{group.name.charAt(0).toUpperCase() + group.name.slice(1)}</Typography>
        {group.data.map(item => (
            <FormControlLabel
                key={item.id}
                control={
